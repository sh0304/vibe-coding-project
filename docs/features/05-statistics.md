...
*   **상세 테이블**: 팀원별 [성명, 직급, 배정 예산, 실 집행액, 잔고]를 표 형태로 제공하며, 기본 정렬은 **직급(Position Level)순**입니다.
...
*   **데이터 안정성 정책**: 
    - 서버와 클라이언트 간의 렌더링 불일치(Hydration Error)를 방지하기 위해 로캘(`ko-KR`) 고정 숫자 포맷팅 함수를 사용합니다.
    - 차트의 Y축 틱(Tick) 및 툴팁 데이터 역시 동일한 포맷팅 규칙이 적용되어야 합니다.
